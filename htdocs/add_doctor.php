<?php
    session_start();
    include('func.php');
    if(isset($_POST['newdoctorsubmit'])){
        $contact = $_POST['contact'];
        $name = $_POST['name'];
        $day = $_POST['day'];
        $time = $_POST['time'];
        $userid=$_SESSION['userid'];
        $query="insert into docstb(contact,name,day,time,userid)values('$contact','$name','$day','$time','$userid');";
            ...
            <div class="col-md-9">
                ...
                <h4>Add Doctor</h4><br>
                  <form method="post" action="add_doctor.php">
                    ...
                    <input type="text" name="name" placeholder="enter doctor name" class="form-control" style="border-radius:20px;"/><br>
                    <input type="text" name="day" placeholder="enter appointment day" class="form-control" style="border-radius:20px;"/><br>
                    <input type="text" name="time" placeholder="enter appointment time" class="form-control" style="border-radius:20px;"/><br>
                    <input type="submit" name="newdoctorsubmit" value="Add" class="btn btn-outline-info" style="border-radius:20px;"/><br>
                  </form>
                </div>
            </div>
        </div>
